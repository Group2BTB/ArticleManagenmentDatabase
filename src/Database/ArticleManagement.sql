...
Source Server         : ArticleManagement
...
-- ----------------------------
DROP IF EXISTS SEQUENCE "id_tbl_user_seq";
...
-- ----------------------------
DROP IF EXISTS SEQUENCE "tbl_article_id_seq";
...
-- ----------------------------
CREATE OR REPLACE FUNCTION "NewProc"(field varchar, order_to varchar, s int4, e int4)
  RETURNS SETOF "pg_catalog"."record" AS $BODY$
   BEGIN
    IF field = 'id' AND order_to = 'ASC' THEN 
      RETURN QUERY
      SELECT 
      ...
      ORDER BY a.id ASC
      LIMIT s OFFSET e; 
    ELSIF field = 'id' AND order_to = 'DESC' THEN 
      RETURN QUERY
      SELECT 
        a.id,a.title,u.fullname,a.create_date
      FROM
      ...
      ORDER BY a.id DESC
      LIMIT s OFFSET e; 
    ELSIF field = 'title' AND order_to = 'ASC' THEN 
      RETURN QUERY
      SELECT 
        a.id,a.title,u.fullname,a.create_date
      FROM
      ...
      LIMIT s OFFSET e; 
    ELSIF field = 'title' AND order_to = 'DESC' THEN 
      RETURN QUERY
      SELECT 
        a.id,a.title,u.fullname,a.create_date
      FROM
      ...
      LIMIT s OFFSET e; 
    ELSIF field = 'author' AND order_to = 'ASC' THEN 
      RETURN QUERY
      SELECT 
        a.id,a.title,u.fullname,a.create_date
      FROM
      ...
      LIMIT s OFFSET e; 
    ELSIF field = 'author' AND order_to = 'DESC' THEN 
      RETURN QUERY
      SELECT 
        a.id,a.title,u.fullname,a.create_date
      FROM
      ...
      LIMIT s OFFSET e; 
    ELSIF field = 'creDate' AND order_to = 'ASC' THEN 
      RETURN QUERY
      SELECT 
        a.id,a.title,u.fullname,a.create_date
      FROM
      ...
      LIMIT s OFFSET e; 
    ELSIF field = 'creDate' AND order_to = 'DESC' THEN 
      RETURN QUERY
      SELECT 
        a.id,a.title,u.fullname,a.create_date
      FROM
...
-- ----------------------------
CREATE OR REPLACE FUNCTION "search_all"(IN _value varchar, IN field varchar, IN order_to varchar, IN s int4, IN e int4)
  RETURNS SETOF "pg_catalog"."record" AS $BODY$
   BEGIN
    IF field = 'id' AND order_to = 'ASC' THEN 
      RETURN QUERY
...
-- ----------------------------
CREATE OR REPLACE FUNCTION "search_by_id"(_value int4, _field varchar, _order_to varchar, _s int4, _e int4)
  RETURNS SETOF "pg_catalog"."record" AS $BODY$
			BEGIN
				IF _field = 'id' AND _order_to = 'ASC' THEN 
						RETURN QUERY
...
-- ----------------------------
CREATE OR REPLACE FUNCTION "search_by_title"(IN _value varchar, IN _field varchar, IN _order_to varchar, IN _s int4, IN _e int4)
  RETURNS SETOF "pg_catalog"."record" AS $BODY$
  BEGIN
				IF _field = 'id' AND _order_to = 'ASC' THEN 
						RETURN QUERY
...
-- ----------------------------
CREATE OR REPLACE FUNCTION "select_all"(IN field varchar, IN order_to varchar, IN s int4, IN e int4)
  RETURNS SETOF "pg_catalog"."record" AS $BODY$
   BEGIN
    IF field = 'id' AND order_to = 'ASC' THEN 
      RETURN QUERY
